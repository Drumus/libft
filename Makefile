# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: mibanez- <mibanez-@student.42barcel>       +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2022/01/13 12:45:03 by mibanez-          #+#    #+#              #
#    Updated: 2022/03/24 13:21:22 by mibanez-         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

NAME = libft.a

SRCS = ft_isalpha.c ft_isdigit.c ft_isalnum.c ft_isascii.c ft_isprint.c \
				ft_toupper.c ft_tolower.c ft_strncmp.c ft_memcmp.c \
				ft_atoi.c ft_strlen.c ft_strlcat.c ft_strlcpy.c ft_strchr.c \
				ft_strrchr.c ft_strnstr.c ft_memset.c ft_memcpy.c ft_memmove.c \
				ft_bzero.c ft_memchr.c ft_calloc.c ft_strdup.c ft_putchar_fd.c \
				ft_putstr_fd.c ft_putendl_fd.c ft_substr.c ft_strjoin.c \
				ft_strtrim.c ft_striteri.c ft_strmapi.c ft_putnbr_fd.c \
				ft_itoa.c ft_split.c 

BONUS = ft_lstnew_bonus.c ft_lstadd_front_bonus.c ft_lstsize_bonus.c \
				ft_lstlast_bonus.c ft_lstdelone_bonus.c ft_lstclear_bonus.c \
				ft_lstiter_bonus.c ft_lstmap_bonus.c ft_lstadd_back_bonus.c \

INCLUDE = libft.h

CC = gcc
RM = rm -f
AR = ar rc
RN = ranlib
CFLAGS = -Wall -Wextra -Werror

.c.o:
			${CC} ${CFLAGS} -c $< -o ${<:.c=.o}

OBJS = ${SRCS:.c=.o}
OBJS_BONUS = ${BONUS:.c=.o}

${NAME}: ${OBJS}
				${AR} ${NAME} ${OBJS}
				${RN} ${NAME}

bonus:	${OBJS} ${OBJS_BONUS}
				${AR} ${NAME} ${OBJS} ${OBJS_BONUS}
				${RN} ${NAME}

all:	$(NAME)

clean:		
			${RM} ${OBJS}

fclean: clean
			${RM} ${NAME}

re: fclean all

.PHONY:	 all clean fclean re
